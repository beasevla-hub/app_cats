  ...
  return (
    <header className="shrink-0 bg-slate-950 px-5 py-4 text-white sm:px-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-1"><button type="button" onClick={onToggleEdit} disabled={!canEdit || saving} className="hidden rounded-lg px-3 py-2 text-xs font-bold text-slate-300 transition hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-40 sm:inline-flex sm:items-center sm:gap-1.5">{editing ? <X size={15} /> : <Edit3 size={15} />}{editing ? "Cancelar" : "Editar"}</button>{editing && <button type="button" onClick={onSave} disabled={saving} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-500 px-3 py-2 text-xs font-bold text-white transition hover:bg-blue-400 disabled:opacity-50"><Save size={15} />{saving ? "Salvando" : "Salvar"}</button>}<button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-400 transition hover:bg-white/10 hover:text-white" aria-label="Fechar"><X size={19} /></button></div>
      </div>
